...
void ClientSocket::SendMsg() {
    ...
    // 초기화
    FD_ZERO(&initset);
    FD_SET(STDIN_FILENO, &initset);  // 표준 입력(STDIN_FILENO)을 감시 대상으로 설정
    ...
    while(1) {
        ...
        // select 함수를 사용하여 표준 입력(STDIN_FILENO) 감시
        tv.tv_sec = 1;
        tv.tv_usec = 0;
        newset = initset;
        ...
        {
            // 영상으로부터 메시지를 입력받음
	    strncpy(msg, g_msg.c_str(), sizeof(msg) - 1);
	    msg[sizeof(msg) - 1] = '\0';  // 문자열의 끝을 나타내는 null 문자 추가
	    //메시지 초기화
	    g_msg = "";
        ...
        // select 함수에서 타임아웃이 발생한 경우, 소켓이 닫혀있으면 함수 종료
        if(ret == 0)
        {
            if(client_socket == -1)
                return;
